...
import { Player, Tournament, Game } from "./ActiveEntities";
...
let state = reactive({
	...
	setUser(user: { name: string; id: string; username: string }) {
		this.user = user;
	},
	...
	setSearchedPlayers(players: Array<Player>) {
		this.searchedPlayers = players
	},

	setGameHistory(history: Array<Game>) {
